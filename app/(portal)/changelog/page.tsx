    ...
    <div className="grid gap-7 xl:grid-cols-[1fr_360px]">
      <div className="min-w-0 space-y-7">
        ...
        <div className="flex gap-8 overflow-x-auto border-b border-white/10 text-sm">
          {["Todas as versões", "Novos recursos", "Melhorias", "Correções", "Depreciações"].map((tab, index) => (
            <Link key={tab} href={`/changelog?tab=${index}`} className={`shrink-0 pb-3 ${index === 0 ? "border-b-2 border-noogym-lime text-white" : "text-noogym-muted"}`}>
              {tab}
            </Link>
          ))}
        </div>
      ...
      <aside className="space-y-5 xl:sticky xl:top-24 xl:self-start">
        ...
        <RightAsideCard title="Fique por dentro">
          ...
          />
          <a href="mailto:developers@noogym.ao?subject=Newsletter%20Changelog%20Noogym" className="lime-button mt-3 w-full py-2.5">Inscrever-se</a>
        ...
        <RightAsideCard title="Versão atual">
          ...
          <Link href="/apis" className="ghost-button mt-4 w-full py-2.5">
        ...
        <RightAsideCard title="Histórico de versões" action={<History className="h-5 w-5 text-white/70" />}>
          ...
          <Link href="/changelog" className="ghost-button mt-4 w-full py-2.5">
